...
file_path = r'c:\Users\pavan\projects\DocSense\docs\DOCSENSE_COMPLETE_DOCUMENTATION.md'
with open(file_path, 'r', encoding='utf-8') as f:
    text = f.read()

new_content = """### 📚 7.3 COMPLETE 43-CATEGORY TREE HIERARCHY (Canonical v8.0)
...
# Find all text between "### 7.3 Complete Category Hierarchy" and "### 7.4 Super-Domain to Parent Group Mapping Table"
pattern = r'(### 7\.3 Complete Category Hierarchy\s*\n\s*```[\s\S]*?```\s*\n)(?=\s*### 7\.4)'

matches = re.finditer(pattern, text)
found = False
for match in matches:
    found = True
    new_text = text[:match.start()] + new_content + "\n\n" + text[match.end():]
