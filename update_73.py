...
content = '''### ?? 7.3 COMPLETE 43-CATEGORY TREE HIERARCHY (Canonical v8.0)
...
for i, line in enumerate(lines):
    if line.startswith('### 7.3 Complete Category Hierarchy'):
        start_idx = i
    if start_idx != -1 and line.startswith('### 7.4 Super-Domain to Parent Group Mapping Table'):
        end_idx = i
        break
...
if start_idx != -1 and end_idx != -1:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.writelines(lines)
    print(f'Successfully updated from line {start_idx+1} to {end_idx}')
else:
